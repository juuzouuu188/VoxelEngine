...
Texture::Texture(
    ...
    //Debugger
    if (!bytes) {
        std::cerr << "Failed to load texture: " << image << ", using fallback green." << std::endl;

        // Generate a 1x1 green pixel texture
        unsigned char greenPixel[4] = { 0, 255, 0, 255 };
        glGenTextures(1, &ID);
        glBindTexture(texType, ID);
        glTexImage2D(texType, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, greenPixel);
